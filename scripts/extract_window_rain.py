...
OUT = "data/processed/window_rain.csv"
STORM_WINDOWS = (1, 3, 7)
STORM_RADIUS_M = 12000
SHIFT_DAYS = 10             # ~1.5 IMERG pixels; matches the finer end of the
...
def main():
    ...
    def per_feature(feat):
        ...
        # Date-tolerant: the wettest window that touches a +/- SHIFT_DAYS span
        # around the catalogue date. The NASA Global Landslide Catalogue is
        # compiled from media reports, which carry the date the slide was
        # written about, not the date the slope moved.
        bands = []
        for days in STORM_WINDOWS:
            best = None
            for shift in range(-SHIFT_DAYS, SHIFT_DAYS + 1, 2):
                end = date.advance(shift, "day")
                window = col["gpm"].filterDate(end.advance(-days, "day"), end)
                half = window.map(lambda img: img.multiply(0.5))
                tot = _or_missing(half, ee.Reducer.sum(), ["r"])
                best = tot if best is None else best.max(tot)
            bands.append(best.rename(f"window_rain_{days}d"))
